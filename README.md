...
Shogun: Total War lets you raise the default battle unit size from 60 to 120 men. This is a battlefield preference — more troops per unit makes engagements feel larger and more cinematic. What it should not do is redesign the campaign the game ships with.

Without this fix, changing unit size to 120 also changes three campaign-side values:
...
## Why this distorts campaign balance

The entire campaign economy in Shogun: Total War was designed, tested, and shipped around 60-man units. The cost of raising an army, the income needed to sustain it, the number of turns required to field trained troops — all of it reflects the 60-man baseline. That design work does not automatically scale correctly when the player changes a graphics preference.

The price and upkeep scaling is purely mechanical: the game reads current unit size from memory and multiplies by it. There is no corresponding rebalancing of province income, building costs, diplomatic penalties, or any other campaign system. The player ends up in a campaign that is more expensive and slower to build armies in without any of the surrounding systems being adjusted to match.

The training-time problem is separate in origin: a hardcoded size threshold that was presumably never tested above 60, and certainly was not tuned to be deliberately harder at that setting.

The battlefield experience of fighting with 120-man units is unchanged by this fix. The fix is strictly about not letting a display preference corrupt the campaign balance the game was built around.
...
| `SupportCostSumTail` | Redirects the support-cost return path to a normalizer code cave. |
...
- The two patch sets write to different file offsets with no overlap.
- They can be applied in either order.
- The combined result is identical regardless of order.
- This patcher explicitly detects whether the audio fix is present and preserves it in all code paths.
